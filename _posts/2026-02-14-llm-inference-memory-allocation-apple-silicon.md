...
$$
\text{kv\_budget} = \text{total\_vram} \times \text{utilization} - \text{weight\_memory} - \text{profile\_peak}
$$
...
$$
\text{ram\_cap} = \begin{cases} \text{system\_ram} \times 2/3 & \text{if } \text{system\_ram} \leq 36\text{ GB} \\ \text{system\_ram} \times 3/4 & \text{otherwise} \end{cases}
$$
...
$$
\text{kv\_budget} = \min(\text{memory\_hint},\; \text{ram\_cap}) \times \text{utilization} - \text{used}
$$
...
$$
\text{KV}_{\text{total}} = n_{\text{kv\_layers}} \times 2 \times n_{\text{kv\_heads}} \times d_{\text{head}} \times n_{\text{ctx}} \times \text{dtype\_size}
$$
